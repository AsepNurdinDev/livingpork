...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 sm:gap-3 shrink-0">
          <Image
            ...
            priority
          />
          <span className="font-sans text-xl sm:text-2xl tracking-wider font-black transition-colors text-stone-900 dark:text-[#fbf7f0]">
            LIVING<span className="text-red-700 dark:text-[#c9a84c]">PORK</span>
          </span>
        </Link>

        {/* Desktop Nav - Diubah dari md:flex menjadi lg:flex agar tablet kebagian Mobile Menu */}
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
          {links.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="text-sm font-semibold tracking-wide transition-colors duration-200 text-stone-600 dark:text-[#fbf7f0]/70 hover:text-red-700 dark:hover:text-[#c9a84c] whitespace-nowrap"
            >
              {link.label}
            </Link>
          ))}
          ...
          <a
            ...
            className="bg-red-700 hover:bg-red-800 text-white text-sm font-bold px-5 py-2.5 rounded-full transition-all duration-200 flex items-center gap-2 shadow-md shadow-red-700/20 hover:scale-105 whitespace-nowrap"
          >
        ...
        {/* Mobile & Tablet: Dark Mode + Hamburger - Diubah dari md:hidden menjadi lg:hidden */}
      ...
      {menuOpen && (
        <>
          {/* Backdrop - Diubah md:hidden menjadi lg:hidden */}
          <div
            className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 lg:hidden"
            onClick={() => setMenuOpen(false)}
          />

          {/* Drawer - Diubah md:hidden menjadi lg:hidden, serta penyesuaian max-width agar rapi di tablet */}
          <div className="fixed top-20 left-4 right-4 z-50 lg:hidden max-w-md mx-auto">
