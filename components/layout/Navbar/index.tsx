...
export default function Navbar() {
  ...
  // 2. Set mounted to true once the client loads
  useEffect(() => {
    setMounted(true)
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/90 dark:bg-[#121212]/95 backdrop-blur-xl shadow-lg shadow-black/5 dark:shadow-black/30 py-3 border-b border-stone-200/60 dark:border-white/5'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          >
            {/* 3. Wrap theme-dependent items in a mounted check */}
            {mounted && darkMode ? (
        ...
        <div className="flex items-center gap-3 md:hidden">
          <button
            ...
            aria-label="Toggle Dark Mode"
          >
            {/* 4. Do the same for the mobile button */}
            {mounted && darkMode ? (
          ...
          <button
            className="text-stone-700 dark:text-[#fbf7f0] focus:outline-none"
